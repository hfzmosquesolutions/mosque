...
export function DependentsTable({ dependents, onEdit, onDelete, loading = false }: DependentsTableProps) {
  ...
  const MobileDependentCard = ({ dependent }: { dependent: UserDependent }) => (
    <div className="border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm hover:shadow-md transition-shadow bg-white dark:bg-slate-900">
      <div className="p-4">
    ...
    <div className="space-y-6">
      ...
      {dependents.length === 0 ? (
        ...
        <DataTable
          ...
          data={dependents}
          searchKey="full_name"
          searchPlaceholder={`${tc('search')} dependents...`}
        />
      )}
